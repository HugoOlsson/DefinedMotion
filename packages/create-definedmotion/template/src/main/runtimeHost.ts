...
export class PersistentRuntimeHost {
  ...
  private readonly projectRoot = resolve(process.cwd())
  ...
  private async processMessage(message: string): Promise<unknown> {
    ...
    return this.enqueue(async () => {
      try {
        ...
        await this.waitForRevision(request.sourceRevision)
          ...
          this.finishRevisionWaiter(waiter, () =>
            rejectPromise(
              new RuntimeHostError(
                ...
                'The source changed, but Vite did not produce a matching ready renderer within 30 seconds'
              )
            )
          )
        }, 30_000)
  ...
  private status(): unknown {
    return {
      ...
      command: 'session',
      action: 'status',
      status: this.rendererRevision ? 'ready' : 'loading',
      ...this.statusMetadata()
    }
  }
