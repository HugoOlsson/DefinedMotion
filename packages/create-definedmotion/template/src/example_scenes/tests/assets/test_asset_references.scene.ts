...
import { AssetRuntimeError, type SceneAsset } from '$renderer/lib/assets/assetReference'
import {
  AnimatedScene,
  HotReloadSetting,
  SceneRuntimeError,
...
export function testAssetReferences(): AnimatedScene {
  return new AnimatedScene(
    ...
    async (scene) => {
      const svg = scene.asset('for_tests/svg/gravity_text.svg')
      ...
      await assertMissingAssetRejected(scene.asset('for_tests/missing-asset.svg'))
