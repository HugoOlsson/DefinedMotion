...
export type AssetNamespace = 'project' | 'package' | 'reference'
...
export class SceneAsset {
  ...
  constructor(path: string, namespace: AssetNamespace = 'project') {
    this.path = validateAssetPath(path)
    this.url = `${DEFINEDMOTION_ASSET_SCHEME}://${namespace}/${this.path
      .split('/')
...
): SceneAsset => new SceneAsset(path, namespace)

/** Creates a reference to a versioned asset shipped by DefinedMotion itself. */
export const packageAsset = (path: string): SceneAsset =>
  createAssetReference(path, 'package')

/** Creates a reference to media shipped specifically for reference scenes and tests. */
export const referenceAsset = (path: string): SceneAsset =>
  createAssetReference(path, 'reference')
...
export const assetPath = (source: AssetSource): string | undefined => {
  ...
  try {
    ...
    if (
      url.protocol !== `${DEFINEDMOTION_ASSET_SCHEME}:` ||
      url.hostname !== 'project' &&
      url.hostname !== 'package' &&
      url.hostname !== 'reference'
    ) {
      return undefined
    }
