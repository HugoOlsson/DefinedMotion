...
export const getProjectAssetRoot = (): string =>
  app.isPackaged ? join(process.resourcesPath, 'assets') : resolve(getProjectRoot(), 'src', 'assets')

export const getReferenceAssetRoot = (): string => {
  const packageRoot = process.env['DEFINEDMOTION_PACKAGE_ROOT']
  if (!packageRoot) throw new Error('DEFINEDMOTION_PACKAGE_ROOT is not configured')
  return resolve(packageRoot, 'reference', 'assets')
}

export const getPackageAssetRoot = (): string => {
  const packageRoot = process.env['DEFINEDMOTION_PACKAGE_ROOT']
  if (!packageRoot) throw new Error('DEFINEDMOTION_PACKAGE_ROOT is not configured')
  return resolve(packageRoot, 'assets')
}

const getAssetRoot = (hostname: string): string => {
  if (hostname === 'project') return getProjectAssetRoot()
  if (hostname === 'package') return getPackageAssetRoot()
  if (hostname === 'reference') return getReferenceAssetRoot()
  throw new Error(`Unknown DefinedMotion asset host: ${hostname}`)
}
