...
- Prepared `latex.morphTo()` destinations are attached after animation binding, which previously forced the scene to reassert overlay material settings during ticks. The formula now uses `scene.addCameraAttachedUI()`, and all morph descendants render in the audience UI pass without per-tick repair.
