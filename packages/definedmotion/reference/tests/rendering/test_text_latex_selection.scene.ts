...
export function testTextLatexSelection(): AnimatedScene {
  return new AnimatedScene(
    ...
    SpaceSetting.TwoDim,
    async (scene) => {
      const panel = createRectangle(72, 40, { color: '#0f172a' })
      panel.position.z = -1
      ...
      scene.verify(
        'latex-selection-in-panel',
        { frames: { start: 0, end } },
        (context) => {
          const contentBounds = context.screenBounds(content)
          const panelBounds = context.screenBounds(panel)
          context.assert(
            containsWithMargin(panelBounds, contentBounds, 8),
            'Text and LaTeX content must remain inside its panel',
            { panelBounds, contentBounds, requiredMargin: 8 }
          )
        }
      )
      scene.verify(
        'latex-selection-rows-separated',
        { frames: { start: 0, end } },
        (context) => {
          const rows = [title, explanation, equation, legend].map((item) =>
      ...
      verifyVisibleMark(scene, 'latex-energy-mark-visible', equation, midpoint(energyStart, energyEnd))
      verifyPartColor(
        scene,
      ...
      verifyPartColor(
        scene,
        'latex-selection-restores-color',
        equation,
        'light',
        '#f8fafc',
        end - 1
...
const midpoint = (start: number, end: number): number =>
