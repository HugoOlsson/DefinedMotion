...
export function chartLayoutPositioningScene(): AnimatedScene {
  return new AnimatedScene(1200, 800, SpaceSetting.TwoDim, async (scene) => {
    ...
    const barBaseline = -7.2
    const barHeights = [6, 9, 12, 8, 14]
    const bars = barHeights.map((height, index) => {
      const root = new THREE.Group()
      root.name = `chart-bar-${index + 1}`
      root.position.set(-10.8 + index * 5.4, barBaseline, 0.5)

      const visual = createRectangle(3.3, height, {
        color: index % 2 === 0 ? '#38bdf8' : '#a78bfa'
      })
      visual.position.y = height / 2
      root.add(visual)
      plotContent.add(root)
      return { root, visual }
    })
    ...
    bars.forEach(({ root }, index) => scene.expose(`chart-bar-${index + 1}`, root))
    ...
    scene.onEachTick((tick) => {
      ...
      bars.forEach(({ root }, index) => {
        root.scale.y = 0.82 + Math.sin(tick * 0.045 + index * 0.65) * 0.18
      })
    })

    scene.addAnims(wait(5))

    scene.verify('chart-bars-stay-on-baseline', {}, (context) => {
      const baselineY = new THREE.Vector3(0, barBaseline, 0).applyMatrix4(plotContent.matrixWorld).y
      const barBottoms = bars.map(({ visual }) => context.worldBounds(visual).min.y)
      context.assert(
        barBottoms.every((bottom) => Math.abs(bottom - baselineY) < 1e-6),
        'Every animated bar must scale from the chart baseline',
        { baselineY, barBottoms }
      )
    })
  })
}
