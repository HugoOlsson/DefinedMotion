...
- `layout.flex()` owns the header, plot captions, and final summary. The complete plots retain explicit centers because they are two fixed mathematical coordinate systems whose authored origins matter more than responsive edge spacing; automatic object measurement is available but is not forced onto this world-space composition.
- The live marker groups are children of their corresponding plots. When a plot is moved, its trace point and guide move with it automatically; the animation continues to author only plot-local mathematical coordinates.
...
- Close viewer inspection exposed a depth invariant missing from the curve primitive: fully opaque curve ribbons had `depthWrite = false`, so translucent plot guides rendered later and painted dark cuts across them. Opaque curves now write ordinary scene depth, while translucent curves retain `depthWrite = false`. The scene consequently needs only small semantic local-Z layers for guides, data, and markers; it no longer mutates materials or disables depth testing.
