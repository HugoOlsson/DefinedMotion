...
- Camera-attached UI is still assembled manually: add ordinary geometry to the camera, add the camera to the scene, then traverse every material to disable depth testing and depth writing. Appended layout children require that material treatment again. A focused camera-attached UI helper would remove repeated, easy-to-forget setup.
